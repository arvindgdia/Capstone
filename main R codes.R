...
setwd("C:/Users/SARVIND2/Documents/WD/Caps1")
con1 <- file("Caps1/final/en_US/en_US.twitter.txt", open = "rb")
twitter <- readLines(con1, skipNul = TRUE, encoding="UTF-8")
close(con1)

con2 <- file("Caps1/final/en_US/en_US.news.txt", open = "rb")
news <- readLines(con2, skipNul = TRUE, encoding="UTF-8")
close(con2)

con3 <- file("Caps1/final/en_US/en_US.blogs.txt", open = "rb")
...
writeLines(SampleAll, "Caps1/SampleAll.txt")
...
SampleAll <- VCorpus(DirSource("Caps1/sampleall", encoding = "UTF-8"))
...
write.csv(quadgram[quadgram$freq > 1,],"Caps1/quadgram.csv",row.names=F)
quadgram <- read.csv("Caps1/quadgram.csv",stringsAsFactors = F)
saveRDS(quadgram,"Caps1/quadgram.RData")
...
write.csv(trigram[trigram$freq > 1,],"Caps1/trigram.csv",row.names=F)
trigram <- read.csv("Caps1/trigram.csv",stringsAsFactors = F)
saveRDS(trigram,"Caps1/trigram.RData")
...
write.csv(bigram[bigram$freq > 1,],"Caps1/bigram.csv",row.names=F)
bigram <- read.csv("Caps1/bigram.csv",stringsAsFactors = F)
saveRDS(bigram,"Caps1/bigram.RData")
